...
Object? _normalizeYaml(Object? value) {
  ...
  if (value is YamlList) return value.map(_normalizeYaml).toList();
  if (value is Map) {
    return <String, dynamic>{
      for (final entry in value.entries)
        entry.key.toString(): _normalizeYaml(entry.value),
  ...
  if (value is List) return value.map(_normalizeYaml).toList();
  return value;
}
...
const _nonProxyTypes = <String>{'direct', 'reject', 'reject-drop', 'pass', 'dns'};
...
Map<String, dynamic> _landingProxy(
  ...
  'udp': settings.udp,
  'dialer-proxy': dialer,
  if (settings.username.trim().isNotEmpty) 'username': settings.username.trim(),
...
void _applyAndroidTunnelPrivacy(Map<String, dynamic> raw) {
  ...
  final hijack = <String>[
    for (final item in (tun['dns-hijack'] as List? ?? const [])) item.toString(),
  ];
...
String? _getRuleProxyTarget(String rule, Set<String> knownTargets) {
  ...
  if (parts[targetIndex].trim().toLowerCase() == 'no-resolve') targetIndex--;
  if (targetIndex < 1) return null;
  final target = parts[targetIndex].trim();
  return knownTargets.contains(target) ? target : null;
}

String _rewriteRuleProxyTarget(String rule, String target, String outboundName) {
  final parts = rule.split(',');
  var targetIndex = parts.length - 1;
  if (parts[targetIndex].trim().toLowerCase() == 'no-resolve') targetIndex--;
...
void _applyDynamicSourceCompatibility(
  ...
  for (final item in proxies) {
    if (item is Map && item['name'] != null) proxyNames.add(item['name'].toString());
  }
  final groupNames = <String>{};
  for (final item in groups) {
    if (item is Map && item['name'] != null) groupNames.add(item['name'].toString());
  }
  ...
  if (usedTargets.isEmpty) throw StateError('当前规则没有可链式代理的代理目标');
...
String applyChainProxyYaml(
  ...
  if (!settings.isComplete) throw StateError('链式代理配置不完整');
  ...
  if (settings.dnsLeakProtection) {
    ...
    if (strictAndroidPrivacy) _applyAndroidTunnelPrivacy(raw);
  }
  if (_hasDynamicProxySources(raw, groups)) {
    _applyDynamicSourceCompatibility(raw, proxies, groups, settings, profileId);
  ...
  for (final item in proxies) {
    if (item is Map && item['name'] != null) existingNames.add(item['name'].toString());
  }
  for (final item in groups) {
    if (item is Map && item['name'] != null) existingNames.add(item['name'].toString());
  }
  ...
  if (chainIndex == 0) throw StateError('当前配置没有可链式代理的节点');
