  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (system.isAndroid && settings.enabled && settings.dnsLeakProtection) {
        ...
        if (!vpnSetting.enable || !vpnSetting.dnsHijacking) {
          ref
        ...
        child: ListView(
          ...
          children: [
            ...
            if (system.isAndroid) ...[
              ...
              const Divider(),
              const SizedBox(height: 8),
              Text('环境一致性', style: Theme.of(context).textTheme.titleMedium),
              const SizedBox(height: 8),
              ListTile(
                contentPadding: EdgeInsets.zero,
              ...
              const Text(
                '这里只显示本机实际暴露给应用的环境值，用于发现网络出口地区和设备环境可能不一致的问题；不会修改或伪造第三方 App 读取到的系统信息。',
                style: TextStyle(fontSize: 13),
              ),
            ],
            const SizedBox(height: 16),
            const Text(
              '普通订阅不会改动原来的规则、地区组或总选择组；每个机场节点仍保留原名称和原选择位置，但实际连接会自动变成“机场节点 → 住宅 SOCKS5”。动态 proxy-provider 配置会自动使用兼容模式。订阅刷新后住宅 SOCKS5 设置仍然保留。',
              style: TextStyle(fontSize: 13),
            ),
          ],
        ),
